import os
from dataset_seg_stage1 import Dataset_train, Dataset_val
...
os.makedirs(os.path.join('trained_models/seg/stage1', save_name), exist_ok=True)
train_writer = SummaryWriter(os.path.join('trained_models/seg/stage1', save_name, 'log/train'), flush_secs=2)
val_writer = SummaryWriter(os.path.join('trained_models/seg/stage1', save_name, 'log/val'), flush_secs=2)
print(save_name)
...
for epoch in range(args.epoch):
    ...
    if epoch + 1 == args.epoch:
        torch.save(net.state_dict(),
                   os.path.join('trained_models/seg/stage1', save_name, 'epoch' + str(epoch + 1) + '.pth'))
    if epoch_val_dice > best_dice:
        best_dice = epoch_val_dice
        torch.save(net.state_dict(),
                   os.path.join('trained_models/seg/stage1', save_name, 'best_dice.pth'))
train_writer.close()
val_writer.close()
